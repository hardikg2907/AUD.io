...
const AudioTile = ({ submission, page, setRender, allSongs, setIsLoading }) => {
  ...
  return (
    <div
      key={submission._id}
      className={`mb-6 hover:shadow-lg transition transform hover:-translate-y-2 cursor-pointer rounded-lg overflow-hidden w-32 h-44 flex justify-center items-center bg-[#3f3f3f] group duration-300 ease-in-out opacity-80 hover:opacity-100 px-2 ${activeSong?._id === submission?._id ? "-translate-y-2 shadow-lg opacity-100" : ""
        }`}
