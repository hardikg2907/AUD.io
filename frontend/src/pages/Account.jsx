...
const Track = ({ index, submission, isFav, submissions }) => {
   ...
   return (
      <div
         className={`grid grid-cols-[1rem_2fr_1fr_1fr_1fr] gap-5 items-center hover:bg-[#333232] p-3 py-4 cursor-pointer ${
            activeSong?._id === submission?._id ? "bg-[#333232]" : ""
         }`}
         onClick={(event) => handleSetSong(submission, submissions, event)}
      >
         ...
         {!isFav ? (
            <div className="flex items-center justify-start gap-2">
               <AiFillEdit />
               <p>{submission?.editAccess.length || 0}</p>
            </div>
         ) : (
            <div className="flex items-center justify-start gap-2">
               <p>{submission?.userId?.username || "Username"}</p>
            </div>
         )}
         <div className="flex items-center justify-start gap-2">
            <AiFillHeart />
            <p>{submission?.likedByUser.length || 0}</p>
         </div>
         <div className="flex items-center justify-start gap-2">
            <AiFillClockCircle />
...
const Account = () => {
   ...
   const fetchData = async () => {
      ...
      if (res) {
         const newData = res?.data.sort(
            (a, b) => b?.likedByUser.length - a?.likedByUser.length
         );
         setSubmissions(newData.slice(0, 4));
      }
      setIsLoading(false);
   };
   ...
   const fetchFavs = async () => {
      ...
      // console.log(res?.data);
      if (res) setFavs(res?.data.slice(0, 4));
      setIsLoading(false);
   };
   ...
   return (
      <div className="w-full h-[87%] mt-5 text-white">
         <div
            className="flex justify-start gap-20 items-center py-8 px-16 rounded-3xl"
            style={{
               background:
                  "linear-gradient(to bottom, #434343 0%, #1E1E1E 100%)",
            }}
         >
            <img
